...
\title[Women's Health and Gender Inequality]{Maternal Mortality and Female Life Expectancy: The Importance of Gender Inequality} 

\author[Bhalotra et al.]{Sonia Bhalotra\inst{\dag}
   \and Damian Clarke\inst{\ddag}
   \and Joseph Gomes\inst{^*} \\
   \and Atheen Venkataramani\inst{\mathsection}}

\institute{\inst{\dag} Essex and ISER
      \and \inst{\ddag} Oxford
      \and \inst{^*} Essex
      \and \inst{\mathsection} Massachusetts General Hospital}
...
\setlength{\itemsep}{20pt}
	\item Maternal mortality is an ``infectious disease condition": 40-50\% of maternal deaths are the result of post-partum puerperal sepsis.
	\item Infectious disease mortality also dominates infant mortality. 
	\item Infant mortality decline started much earlier and progressed more rapidly than maternal mortality decline.
	\item The hypothesis is that this is a function of gender prejudice.
\end{itemize}
}
...
\setlength{\itemsep}{20pt}
	...
	\setlength{\itemsep}{10pt}
		...
		\item We home in on maternal mortality and assess the role of ``cultural factors" conditional upon income (Jayachandran 2014 discusses this in more general terms).
	\end{itemize}
\end{itemize}
}
...
\begin{itemize}
\setlength{\itemsep}{20pt}
	\item So the hypothesis we'd like to test is that the rate of decline of maternal mortality is a function of gender prejudice.
	\item Mechansim?
	\begin{itemize}
	\setlength{\itemsep}{15pt}
		...
		\begin{itemize}
			\item We use TB as a placebo disease.
		\end{itemize}
	\end{itemize}
\end{itemize}
}
...
\frame{
\frametitle{MMR: Only recent policy initiatives, varying adoption}
\begin{itemize}
\setlength{\itemsep}{20pt}
	...
	\item Limited progress, substantial declines only in the 2000s led to the UN Secretary General launching the Every Woman Every Child Initiative and creating the Commission on Information and Accountability for Women and Children’s Health in 2010 (Kassebaum et al 2014).
\end{itemize}
}
...
\begin{itemize}
\setlength{\itemsep}{20pt}
	\item The challenge is to find exogenous change in gender inequality.
	...
	\item Here we use two approaches
		\begin{itemize}
			\item Premise that \textbf{language structure} embeds gender differentiation. Use variation in language at birth to proxy deep-set (centuries old) gender attitudes.
			\item Use cohort-country variation in stated son preference and in institutionalized political, economic and social rights of women, relying upon panel data identification.
		\end{itemize}
\end{itemize}
}
...
\section{Data and Descriptive Stats}
\frame{
\frametitle{Data}
Collate and create a range of indicators on female health, general health, and gender bias
\vspace{6mm}
\begin{itemize}
\item Country$\times$year measures of life expectancy, MMR$^{*}$, TB, mortality
\item Gender bias
    \begin{enumerate}
      \item political rights, economic right
      \item seats held by women in parliament
      \item desired gender ratio of children (from large household survey)
      \item gender bias embedded in language
    \end{enumerate}
\item Historical data on MMR declines in USA
\end{itemize}
}
...
\frame{\frametitle{Simple Correlated Trends}
\begin{itemize}
\item Female (and male) life expectancy \hyperlink{FLExp}{\textcolor{blue}{strongly correlated}} with income
\item But, F-M life expectancy \emph{differential} \hyperlink{FLExp}{\textcolor{blue}{much less so}}
\item This life expectancy differential \emph{is} \hyperlink{dFLExpBias}{\textcolor{blue}{related to gender bias}}
\item While life expectancy itself is \hyperlink{FLExpBias}{\textcolor{blue}{less}} related to gender bias
\item MMR is \hyperlink{MMRBias}{\textcolor{blue}{also related}} to gender bias
\end{itemize}
}

\begin{frame}[label=FLExp,plain]
\begin{figure}
\caption{Female Life Expectancy and ln(GDP)}
\includegraphics[scale=0.8]{./figures/lLEGDP2010.eps}
\end{figure}
\end{frame}

\begin{frame}[dFLExp,plain]
\begin{figure}
...
\begin{itemize}
\setlength{\itemsep}{20pt}
	\item Grammatical gender can reflect gender attitudes in society
	\begin{itemize}
...
\begin{frame}
\frametitle{Gender Intensity of Language and Abortion Legalisation}
...
\begin{itemize}
	...
	\begin{itemize}
		\item Political, Economic \& Social rights. %rights to vote, run for political office, hold elected and appointed government positions, join political parties, petition government officials. 
...
%\_cons      &      -844.8\sym{*}  &      -193.6         &      -464.1         &     -1458.7\sym{***}&     -3322.4\sym{***}\\
...
Democracy     &                     &                     &      -10.02\sym{***}&                     &      -6.489\sym{*}  &      -72.51\sym{***}\\
            &                     &                     &     (3.686)         &                     &     (3.487)         &     (14.98)         \\
Rights * log GDP   &                     &                     &                     &       47.47\sym{***}&       45.42\sym{***}&       35.17\sym{***}\\
            &                     &                     &                     &     (6.780)         &     (8.244)         &     (8.257)         \\
Democracy * log GDP   &                     &                     &                     &                     &                     &       9.658\sym{***}\\
            &                     &                     &                     &                     &                     &     (2.090)         \\
%\_cons      &      1018.8\sym{***}&       300.8\sym{*}  &       308.0\sym{*}  &       996.5\sym{***}&       976.8\sym{***}&      1071.0\sym{***}\\
...
%\hline\hline
            ...
            %%&\multicolumn{1}{c}{indep1}&\multicolumn{1}{c}{indep2}&\multicolumn{1}{c}{indep3}&\multicolumn{1}{c}{indep4}&\multicolumn{1}{c}{indep5}\\
%\hline
%Economic Rights&   -0.813         &       14.25         &       6.624         &      -168.2\sym{*}  &      -164.6\sym{*}  &      -103.7         \\
